...
app.get('*', async(req, res) => {
  ...
  [finalHtml] = await Promise.all([fetch(queryUrl)
    ...
    .then(claim => {
      ...
      const claimUrl = `https://beta.lbry.tv/${claim.name}${claim.claim_id ? (`/${claim.claim_id}`) : ''}` || 'https://beta.lbry.tv';
